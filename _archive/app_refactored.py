"""
MARBEFES BBT Database - Marine Biodiversity and Ecosystem Functioning Database
"""
...
# ============================================================================
# REFACTORED HELPER FUNCTIONS (Optimization 1 & 2: JSON Loading)
# ============================================================================

def load_json_data(file_path, data_key=None, description="data"):
    """
    Generic JSON file loader with standardized error handling.

    Args:
        file_path: Path to JSON file
        data_key: Optional key to extract from loaded JSON (e.g., 'statistics', 'bbts')
    ...
    if not file_path.exists():
        logger.info(f"{description.capitalize()} file not found at {file_path}")
        return {}
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if isinstance(data, dict):
                if 'metadata' in data and 'bbt_count' in data['metadata']:
                    logger.info(f"Loaded {description} for {data['metadata']['bbt_count']} BBT areas")
                elif 'bbts' in data:
                    logger.info(f"Loaded {description} for {len(data['bbts'])} BBT areas")
                else:
...
def load_bathymetry_stats():
    ...
    return load_json_data(
        BATHYMETRY_STATS_FILE,
        data_key='statistics',
        description="bathymetry statistics"
    )
...
# ============================================================================
# REFACTORED HELPER FUNCTIONS (Optimization 1: WMS Health Check)
# ============================================================================

def check_service_health(service_url, service_name="WMS"):
    """
    Check health of a WMS service with standardized error handling.

    Args:
        service_url: Base URL of the WMS service
        service_name: Name of service for logging (default: "WMS")
    ...
    try:
        ...
        response = wms_session.get(service_url, params=params, timeout=3)
...
# ============================================================================
# REFACTORED HELPER FUNCTIONS (Optimization 3: WMS Capabilities Fetching)
# ============================================================================

def fetch_wms_layers(base_url, filter_fn=None, service_name="WMS"):
    """
    Fetch and parse WMS capabilities with standardized error handling.

    Args:
        base_url: WMS service base URL
        filter_fn: Optional layer filter function
        service_name: Name for logging

    Returns:
        list: Parsed WMS layers or empty list on error
    """
    try:
        logger.info(f"Fetching {service_name} capabilities from {base_url}")

        params = {"service": "WMS", "version": WMS_VERSION, "request": "GetCapabilities"}
        response = wms_session.get(base_url, params=params, timeout=WMS_TIMEOUT)

        if response.status_code == 200:
            layers = parse_wms_capabilities(response.content, filter_fn=filter_fn)
            logger.info(f"Successfully fetched {len(layers)} {service_name} layers")
            return layers
        else:
            logger.warning(f"{service_name} request failed with status {response.status_code}")
            return []

    except requests.RequestException as e:
        logger.warning(f"Network error fetching {service_name} layers: {e}")
        return []
    except ET.ParseError as e:
        logger.error(f"XML parsing error in {service_name} response: {e}")
        return []
    except Exception as e:
        logger.error(f"Unexpected error fetching {service_name} layers: {e}", exc_info=True)
        return []


# ============================================================================
# REFACTORED DECORATOR (Optimization 4: Vector Support Checks)
# ============================================================================

def require_vector_support(f):
    """
    Decorator to require vector support for endpoint.
    Returns 503 error if vector support is not available.
    """
    @wraps(f)
...
# ============================================================================
# REFACTORED HELPER FUNCTION (Optimization 5: Error Response Pattern)
# ============================================================================

def api_error_response(error_message, status_code=500, **extra_fields):
    """
...
def get_available_layers():
    """
    Fetch available WMS layers with caching (REFACTORED)

    Returns:
        List of layer dictionaries
    """
    # Use refactored fetch helper
    wms_layers = fetch_wms_layers(WMS_BASE_URL, filter_fn=emodnet_layer_filter, service_name="EMODnet")

    if wms_layers:
        return prioritize_european_layers(wms_layers)
    else:
        logger.warning("Using fallback EMODnet layers")
        return EMODNET_LAYERS


def get_helcom_layers():
    """Fetch available layers from HELCOM WMS GetCapabilities (REFACTORED)"""
    return fetch_wms_layers(HELCOM_WMS_BASE_URL, filter_fn=helcom_layer_filter, service_name="HELCOM")
...
# ============================================================================
# Flask route handlers (REFACTORED to use helper functions)
# ============================================================================
...
def health_check():
    """
    Health check endpoint for monitoring and load balancers (REFACTORED)
    ...
    # Check WMS connectivity (REFACTORED - using helper function)
    wms_healthy, wms_error = check_service_health(WMS_BASE_URL, "EMODnet WMS")
    ...
    # Check HELCOM WMS connectivity (REFACTORED - using helper function)
    helcom_healthy, helcom_error = check_service_health(HELCOM_WMS_BASE_URL, "HELCOM WMS")
...
@require_vector_support  # REFACTORED - using decorator
def api_vector_layers():
    """API endpoint to get available vector layers (REFACTORED)"""
    try:
        vector_layers = get_vector_layers_summary()
        return jsonify({"layers": vector_layers, "count": len(vector_layers)})
    except Exception as e:
        logger.error(f"Error in api_vector_layers: {e}", exc_info=True)
        return api_error_response(str(e))  # REFACTORED - using helper
...
def api_vector_layer_geojson(layer_name):
    """API endpoint to get GeoJSON for a specific vector layer (REFACTORED)"""
    ...
    if vector_loader and vector_loader.loaded_layers:
        ...
        if layer_name not in valid_layer_names:
            ...
            return api_error_response(f"Layer '{layer_name}' not found", status_code=404)
...
def api_vector_bounds():
    """API endpoint to get bounds of all vector layers (REFACTORED)"""
    try:
...
def api_factsheets():
    """API endpoint to get all BBT factsheet data (cached in memory) (REFACTORED)"""
    try:
        # Use cached data loaded at startup for performance
        if not FACTSHEET_DATA:
            return api_error_response("Factsheet data not found", status_code=404)
...
def api_factsheet(bbt_name):
    """API endpoint to get factsheet data for a specific BBT (cached in memory) (REFACTORED)"""
    try:
        # Use cached data loaded at startup for performance
        if not FACTSHEET_DATA:
            return api_error_response("Factsheet data not found", status_code=404)
        ...
        return api_error_response(f"No factsheet found for BBT: {bbt_name}", status_code=404)
...
def api_capabilities():
    """API endpoint to get WMS capabilities (REFACTORED)"""
    params = {"service": "WMS", "version": WMS_VERSION, "request": "GetCapabilities"}
    try:
...
def api_metrics():
    ...
    try:
        ...
        if not data or 'metrics' not in data:
            return api_error_response("No metrics provided", status_code=400)
